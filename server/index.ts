...
const CLIENT_ID = process.env.POSTY_OAUTH_CLIENT_ID!;
const CLIENT_SECRET = process.env.POSTY_OAUTH_CLIENT_SECRET!;
const FRONTEND_URL = process.env.POSTY_FRONTEND_URL || 'https://platform.posty.hu';
const API_URL = process.env.POSTY_API_URL || 'https://api.posty.hu';
const SERVER_URL = process.env.SERVER_URL || `http://localhost:${PORT}`;
const DATABASE_URL = process.env.DATABASE_URL!;

if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error('POSTY_OAUTH_CLIENT_ID and POSTY_OAUTH_CLIENT_SECRET are required');
  process.exit(1);
}
...
function html(res: ServerResponse, status: number, body: string) {
  ...
  res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Posty CLI Auth</title>
...
async function handleVerifySubmit(req: IncomingMessage, res: ServerResponse) {
  ...
  // Redirect to Posty OAuth with device_code in state so we can match on callback
...
// GET /device/callback — Posty redirects here after OAuth authorization
...
async function start() {
  ...
  server.listen(PORT, () => {
    console.log(`Posty CLI auth server running on ${SERVER_URL}`);
    console.log(`OAuth callback URL (configure in Posty): ${SERVER_URL}/device/callback`);
  });
}
