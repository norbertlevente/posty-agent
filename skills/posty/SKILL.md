...
Never run `npm install`, never install or invoke the `posty` command line
tool, and never touch the user's shell for Posty work unless the user
explicitly asks for a CLI or script workflow. The MCP tools cover every task
below without leaving the conversation.
...
## The command line tool, only on request

Posty also ships `posty-cli` (`npm install -g posty-cli`, the command is
`posty`) for shells, scripts and CI. It signs in separately with
`posty auth:login`. Use it only when the user asks for a terminal or script
workflow; the full command reference is in the package's own SKILL.md and in
the repository README at https://github.com/norbertlevente/posty-agent.
